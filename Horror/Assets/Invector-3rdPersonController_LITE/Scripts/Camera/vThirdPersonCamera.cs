    ...
    public void RotateCamera(float x, float y)
{
    ...
    if (!lockCamera)
    {
        ...
        // Smoothly clamp the horizontal rotation to avoid abrupt changes near the 360-degree point
        mouseX = Mathf.Clamp(mouseX, xMinLimit, xMaxLimit);

        // Add a smoothing effect to reduce shaking when near extreme angles
        Quaternion targetRotation = Quaternion.Euler(mouseY, mouseX, 0);
        targetLookAt.rotation = Quaternion.Slerp(targetLookAt.rotation, targetRotation, Time.deltaTime * smoothCameraRotation);
    }
    else
    {
        // If the camera is locked, directly set the rotation to match the target
